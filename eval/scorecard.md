...
_Mode: routing-only (deterministic baseline router, no LLM calls). Read this number with suspicion: the baseline's patterns were written against these same questions, so it is fitted to them and its score is an upper bound, not a generalization estimate. · generated 2026-09-07 22:55 UTC by `python -m eval.run_golden_set`._
